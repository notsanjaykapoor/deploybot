...
use super::slack::{SlackMessage, SlackMessageSend};
...
pub struct DeployStage {
    ...
    pub sender: crossbeam::Sender<String>,
}

impl DeployStage {

    pub fn new(id: String, repo: String, tag: String, path: String, logger: slog::Logger, sender: crossbeam::Sender<String>) -> DeployStage {
        DeployStage {
            id: id,
            repo: repo,
            tag: tag,
    ...
    pub async fn call(&mut self) -> Option<i32> {
    ...
    fn _slack_message(&self, subject: &str, state: &str) -> Option<i32> {
        ...
        match SlackMessageSend::call(&self.sender, &message) {
            Err(e) => {
